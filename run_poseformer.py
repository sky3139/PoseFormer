...
dataset_path = '/home/u20/d2/code/VideoPose3D/data/data_3d_h36m.npz'
path_2d="/home/u20/d2/code/VideoPose3D/data/data_2d_h36m_gt.npz"
...
for subject in dataset.subjects():
    ...
    for action in dataset[subject].keys():
        ...
        for cam_idx in range(len(keypoints[subject][action][1])):

            # We check for >= instead of == because some videos in H3.6M contain extra frames
            mocap_length = dataset[subject][action]['positions_3d'][cam_idx].shape[0]
            assert keypoints[subject][action][1][cam_idx].shape[0] >= mocap_length

            if keypoints[subject][action][1][cam_idx].shape[0] > mocap_length:
                # Shorten sequence
                keypoints[subject][action][1][cam_idx] = keypoints[subject][action][1][cam_idx][:mocap_length]

        assert len(keypoints[subject][action][1]) == len(dataset[subject][action]['positions_3d'])

for subject in keypoints.keys():
    for action in keypoints[subject]:
        for cam_idx, kps in enumerate(keypoints[subject][action][1]):
            # Normalize camera frame
            cam = dataset.cameras()[subject][cam_idx]
            kps[..., :2] = normalize_screen_coordinates(kps[..., :2], w=cam['res_w'], h=cam['res_h'])
            keypoints[subject][action][1][cam_idx] = kps
...
def fetch(subjects, action_filter=None, subset=1, parse_3d_poses=True):
    ...
    for subject in subjects:
        for action in keypoints[subject].keys():
            ...
            poses_2d = keypoints[subject][action][1]
